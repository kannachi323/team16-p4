    ...
    def successorStates(self, state):
        ...
        for action in Directions.CARDINAL:
            ...
            if (not hitsWall):
                ...
                successor = ((nextState, newVisitedCorners), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    for corner in unvisitedCorners:
        manhattanDistances.append(manhattan(corner, currentPosition))
...
def foodHeuristic(state, problem):
    ...
    for item in set(food_list):
        foodDistance = manhattan(position, item)
        heuristic = max(foodDistance, heuristic)
    return heuristic
